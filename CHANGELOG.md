...
- Publish flow now shows webhook error notification when the remote returns an HTTP error (e.g. 422)

## [1.2.2] - 2026-05-12

### Fixed

- Post title and "Updated" timestamp no longer merge on the same line in the documents table
- Webhook trigger button now correctly shows error notification when the remote returns an HTTP error (e.g. 404)

## [1.2.0] - 2026-05-11

### Added

- Manual webhook trigger button in Settings page (matching native Strapi webhook UI)
- `POST /bulk-publish/trigger` API endpoint for manual webhook invocation
